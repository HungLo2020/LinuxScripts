...
prepare_external_data_dir() {
    ...
    local probe_file="${path}/.nc-write-probe"
    ...
    if sudo -u "#33" bash -c "touch \"${probe_file}\" && rm -f \"${probe_file}\"" >/dev/null 2>&1; then
        return 0
    fi

    log "Error: Nextcloud webserver user (uid 33) cannot write to ${path}."
    log "For CIFS/Samba mounts, remount with options like: uid=33,gid=33,file_mode=0664,dir_mode=0775"
    exit 1
}

write_shared_storage_override_config() {
    local config_dir="${NEXTCLOUD_APP_DIR}/config"
    local override_file="${config_dir}/zz-shared-storage.config.php"
