...
Server capabilities include Btrfs snapshot management for `/srv/storage`, plus legacy-compatible Restic, ZIP, and public GitHub backup managers. Restic configurations, generated helpers, and password files remain under `~/.config/restic-mattmc/`; ZIP configurations and helpers remain under `~/.config/zip-backup-manager/`. Their systemd timers run as the user who configures them. The GitHub backup manager installs its persistent six-hour systemd timer and stores its working state under the configured paths in `src/server/github_backups.py`. The Server Manager provides Debian repository management: explicitly select MattOS or MattPackages, then setup, status, list, verify, or publish. Both repositories run in one systemd service with separate local roots and R2 buckets; MattPackages starts empty and shares the existing MattOS signing key. See [server management](../Docs/ServerManagement.md).
