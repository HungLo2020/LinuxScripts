#!/usr/bin/env bash
# =============================================================================
# RunVaultwardenContainer.sh
#
# Self-contained script to install and run Vaultwarden (Bitwarden-compatible
# password manager) via Docker.
#
# Usage:
#   ./RunVaultwardenContainer.sh          Install (if needed) and start
#   ./RunVaultwardenContainer.sh -D       Stop container and delete all files/image
#   ./RunVaultwardenContainer.sh --off    Stop container without deleting files
#   ./RunVaultwardenContainer.sh --on     Start container only if already installed
#
# Notes:
#   - Web vault UI is exposed on http://localhost:8222
#   - Persistent data: ~/.vaultwarden
#   - Use any Bitwarden-compatible client to connect
#   - Admin panel: http://localhost:8222/admin (requires ADMIN_TOKEN)
# =============================================================================
...
BASE_DATA_DIR="${HOME}/.vaultwarden"
...
# ── -D delete mode ────────────────────────────────────────────────────────────
if [[ "${ACTION}" == "delete" ]]; then
    log "=== Shutting down Vaultwarden and removing files ==="
    ...
    if docker_exec images -q "${IMAGE_NAME}" 2>/dev/null | grep -q .; then
        log "Removing Vaultwarden image..."
...
if [[ "${ACTION}" == "on" ]]; then
    if ! docker_exec images -q "${IMAGE_NAME}" 2>/dev/null | grep -q .; then
        log "Error: Vaultwarden image is not installed. Run without flags first."
        exit 1
    fi
    if [[ ! -d "${DATA_DIR}" ]]; then
        log "Error: Vaultwarden data directory is not installed. Run without flags first."
        exit 1
    fi
fi
...
ADMIN_TOKEN_FILE="${BASE_DATA_DIR}/admin_token.txt"
...
if container_exists "${CONTAINER_NAME}"; then
    ...
    log "Admin token saved to: ${ADMIN_TOKEN_FILE}"
fi

for _ in {1..60}; do
    if curl -fsS "http://127.0.0.1:${WEB_PORT}/" >/dev/null 2>&1; then
        log "=== Vaultwarden is ready ==="
        log "Web vault:   http://localhost:${WEB_PORT}"
        log "Admin panel: http://localhost:${WEB_PORT}/admin"
        log "Admin token: $(cat "${ADMIN_TOKEN_FILE}" 2>/dev/null || echo '(see ~/.vaultwarden/admin_token.txt)')"
...
log "Vaultwarden container started, but readiness check timed out."
