...
BISYNC_CMD="/usr/bin/flock -n \"$LOCK_FILE\" /usr/bin/rclone bisync \"$RCLONE_REMOTE\" \"$ONEDRIVE_DIR\" --config \"$RCLONE_CONFIG\" --check-access --verbose >> \"$BISYNC_LOG\" 2>&1"
RESYNC_CMD="/usr/bin/flock -n \"$LOCK_FILE\" /usr/bin/rclone bisync \"$RCLONE_REMOTE\" \"$ONEDRIVE_DIR\" --config \"$RCLONE_CONFIG\" --resync --check-access --verbose >> \"$RESYNC_LOG\" 2>&1"
...
run_as_target_user rclone bisync "$RCLONE_REMOTE" "$ONEDRIVE_DIR" --config "$RCLONE_CONFIG" --resync --check-access --verbose
